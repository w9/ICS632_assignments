...
int main(int argc, char *argv[])
{
  ...
  /* printf("bcast_implementation_name = %s\n", bcast_implementation_name); */
  /* printf("chunk_size = %d\n", chunk_size); */
  /* printf("NUM_BYTES = %d\n", NUM_BYTES); */
  ...
  if (!strcmp(bcast_implementation_name, "naive_bcast")) {
  ...
  } else if (!strcmp(bcast_implementation_name, "default_bcast")) {
    MPI_Bcast(buffer, NUM_BYTES, MPI_CHAR, 0, MPI_COMM_WORLD);

  } else if (!strcmp(bcast_implementation_name, "ring_bcast")) {
    if (rank > 0) {
#if OUTPUT_TIMELINE == 1
      double tic = MPI_Wtime();
#endif
    ...
    for (i = 0; i < NUM_BYTES; i += chunk_size) {
      j = chunk_size > NUM_BYTES - i ? NUM_BYTES - i : chunk_size;
      if (rank > 0) {
        MPI_Recv(buffer + i, j, MPI_CHAR, rank - 1, 0, MPI_COMM_WORLD, &status);
      }
      if (rank < num_procs - 1) {
        MPI_Send(buffer + i, j, MPI_CHAR, rank + 1, 0, MPI_COMM_WORLD);
      }
    }
    ...
    for (i = 0; i < NUM_BYTES; i += chunk_size) {
      j = chunk_size > NUM_BYTES - i ? NUM_BYTES - i : chunk_size;
      if (rank > 0) {
        MPI_Recv(buffer + i, j, MPI_CHAR, (rank - 1) / 2, 0, MPI_COMM_WORLD, &status);
      }
      if (i > 0) {
  ...
  if ((0 == rank) && (all_ok)) {
#if OUTPUT_TIME == 1
    printf("%.3lf", MPI_Wtime() - start_time);
#endif

    fprintf(stderr,"implementation: %s | chunksize: %d |  time: %.3lf seconds\n",
		    bcast_implementation_name,
		    chunk_size,
